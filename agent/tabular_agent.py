...
class TabularAgent():
    ...
    def train_step(self, obs_arr, action, reward, next_obs_arr):
        '''
        Updates q values
        ...
        parameters:
            ...
            action : (int)
            reward : (float)
            next_obs_arr : (np.array) with a single entry giving the index of the next state
        '''
        obs = obs_arr[0]
        next_obs = next_obs_arr[0]
        self.q[obs, action] += self.step_size*(reward + self.gamma*np.max(
            self.q[next_obs, :]) - self.q[obs, action])  # Q-learning update
